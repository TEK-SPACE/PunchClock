    ...
    public partial class PunchDbInitializer 
    {
        private void SeedAppSettings(PunchClockDbContext context)
        {
            List<AppSetting> appSettings = new List<AppSetting>
            {
                ...
                new AppSetting {Id =8, GlobalId = Guid.NewGuid(), IsPrivate = false,ModuleId = (int) ModuleType.Core, Key = "CoreUserRegistrationTimeZoneDefault", Value = "India Standard Time", ValueType = KeyValueType.String,  Description = "Valid Time zone Ex: India Standard Time", IsEditable = true}
            };
