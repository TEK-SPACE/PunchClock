    ...
    public partial class PunchDbInitializer
    {
        private void SeedTicketType(PunchClockDbContext context)
        {
            var tickettype = new List<TicketType>
            {
                ...
                new TicketType {Id = 1, Name = "Question",Description ="",DisplayOrder =1,CompanyId = context.Companies.First().Id},
                //Enhancement: a new feature or an improvement over an existing one
                new TicketType {Id = 2, Name = "Incident",Description ="",DisplayOrder =2,CompanyId = context.Companies.First().Id},
                //Task: everything else, something that needs to be done
                new TicketType {Id = 3, Name = "Task",Description ="",DisplayOrder =3,CompanyId = context.Companies.First().Id},
                //Problem is used to indicate that the requester is having an issue with your product or service that needs to be resolved
                new TicketType {Id=4,Name = "Problem",Description = "",DisplayOrder =4,CompanyId = context.Companies.First().Id}
            };
